...
class UpgradeScene extends Phaser.Scene {
    ...
    create(){
        ...
        var date = new Date()
        if(gameState.thingsToSave.date !== date.getDate()){
            gameState.thingsToSave.date = date.getDate();
            gameState.save();
            for (var i = 0; i < 3; i++){
                var randHat = Math.ceil(Math.random()*gameState.items.length)-1;
                gameState.thingsToSave.shopItems.push(gameState.items[randHat]);
            }
        }
        console.log(gameState.thingsToSave.shopItems.length);
        if(gameState.thingsToSave.shopItems.length !== 0){
            for (var i = 0; i < 3; i++){
                this.add.image((i+1)*100,350,`${gameState.thingsToSave.shopItems[i].name}`).setDepth(10);
            }
        }
    }
    update(){
